# filepath: /home/msalsouri/Projects/DB-GPT/docker/cc-ollama/install-ccgpt-full.sh
#!/bin/bash
# Complete installation script for CodexContinue-GPT
# This script creates a working CodexContinue-GPT setup in the container
...
echo -e "${CYAN}====================================${NC}"
echo -e "${CYAN}CodexContinue-GPT Full Installer    ${NC}"
echo -e "${CYAN}====================================${NC}"
echo ""
...
# Create the script directly in the container
echo -e "Creating the main script..."
docker exec -it dbgpt-shell-agent bash -c "cat > /app/agent/codexcontinue-gpt.sh << 'EOF'
#!/bin/bash
...
6. Example code for any suggested improvements\"
    ...
    # Auto-select the best model for code auditing if auto is on
...
4. How can I improve my task management approach?\"
        ...
        # Auto-select the best model for task management if auto is on
...
chat_mode() {
    ...
    while true; do
        ...
        # Process the query (with simple error handling)
        response=\$(echo \"\$user_input\" | ollama run \$current_model --system \"\$SYSTEM_PROMPT\" 2>&1)
        if [ \$? -ne 0 ]; then
            echo -e \"\${RED}Error running Ollama. Using default response.\${NC}\"
            response=\"I encountered an issue with the Ollama model. Please check that Ollama is running properly.\"
        fi
...
EOF
chmod +x /app/agent/codexcontinue-gpt.sh"

# Create the command wrappers
echo -e "Creating command wrappers..."
docker exec -it dbgpt-shell-agent bash -c "cat > /usr/local/bin/ccgpt << 'EOW'
#!/bin/bash
exec /app/agent/codexcontinue-gpt.sh \"\$@\"
EOW
chmod +x /usr/local/bin/ccgpt

cat > /usr/local/bin/cc << 'EOW'
#!/bin/bash
exec /app/agent/codexcontinue-gpt.sh \"\$@\"
EOW
...
docker exec -it dbgpt-shell-agent bash -c "grep -q 'alias cc=' ~/.bashrc || echo \"
