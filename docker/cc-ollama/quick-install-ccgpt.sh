# filepath: /home/msalsouri/Projects/DB-GPT/docker/cc-ollama/quick-install-ccgpt.sh
#!/bin/bash
# One-line installation command - copy and paste this into your terminal
# This creates a minimal CodexContinue-GPT implementation focused on auto-model selection

docker exec -i dbgpt-shell-agent bash -c 'cat > /app/agent/ccgpt.sh << "EOF"
#!/bin/bash
# CodexContinue-GPT - Simple Auto-Model Selection Shell Agent
...
# Create simpler CLI commands
echo "#!/bin/bash" > /usr/local/bin/cc
echo "exec /app/agent/ccgpt.sh \"\$@\"" >> /usr/local/bin/cc
chmod +x /usr/local/bin/cc

echo "#!/bin/bash" > /usr/local/bin/ccgpt
echo "exec /app/agent/ccgpt.sh \"\$@\"" >> /usr/local/bin/ccgpt
chmod +x /usr/local/bin/ccgpt

echo "CodexContinue-GPT installed successfully!"
echo "Run with: cc --auto"
'
