# filepath: /home/msalsouri/Projects/DB-GPT/docker/cc-ollama/install-ccgpt-complete.sh
#!/bin/bash
# Install CodexContinue-GPT with full auto-model selection
...
echo -e "${CYAN}====================================${NC}"
echo -e "${CYAN}CodexContinue-GPT Complete Installer${NC}"
echo -e "${CYAN}====================================${NC}"
echo ""
...
echo -e "Installing complete version of CodexContinue-GPT..."

# Copy the complete script to the container
echo -e "\n${YELLOW}Copying complete script to container...${NC}"
docker cp ccgpt-complete.sh dbgpt-shell-agent:/app/agent/ccgpt.sh
docker exec -it dbgpt-shell-agent bash -c "chmod +x /app/agent/ccgpt.sh"

echo -e "${GREEN}✓ Complete script copied successfully${NC}"
...
echo "   cc --auto"
echo ""
echo "3. Run in test mode (without real Ollama calls):"
echo "   cc --auto --test"
echo ""
echo "4. Or specify a model manually:"
