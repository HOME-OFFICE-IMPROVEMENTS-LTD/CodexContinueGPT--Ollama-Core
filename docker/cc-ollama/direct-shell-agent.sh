# filepath: /home/msalsouri/Projects/DB-GPT/docker/cc-ollama/direct-shell-agent.sh
#!/bin/bash
...
# Function to automatically select the best model based on input
auto_select_model() {
    local input="$1"
    
    # Simple heuristics to determine the task type
    if [[ "$input" == *"error"* || "$input" == *"bug"* ]]; then
        echo "codellama" # Code-related task
    elif [[ "$input" == *"task"* || "$input" == *"manage"* ]]; then
        echo "mistral" # Task management
    else
        echo "llama3" # General conversation
    fi
}
...
chat_mode() {
    ...
    while true; do
        ...
        # Exit if requested
        if [[ "$user_input" == "exit" ]]; then
            echo "Exiting shell agent. Goodbye!"
            break
        fi
